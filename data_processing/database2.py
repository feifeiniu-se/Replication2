...
def read_scores(path, issues):
    ...
    for tmp in result:
        if tmp[0] in issue_map:
            ...
            issue.bluir_score[tmp[1]] = float(float(tmp[2]) - Min)/(Max - Min)
    ...
    for tmp in result:
        if tmp[0] in issue_map:
            issue = issue_map[tmp[0]]
            issue.simi_score[tmp[1]] = float(float(tmp[2]) - Min)/(Max - Min)
    ...
    for tmp in result:
        if tmp[0] in issue_map:
            ...
            if tmp[1] in issue.simi_score or tmp[1] in issue.bluir_score:
                issue.cache_score[tmp[1]] = float(float(tmp[2]) - Min)/(Max - Min)
...
#     for issue in issues:
#         if issue.issue_id == 'HORNETQ-106':
#             for key, value in issue.bluir_score.items():
#                 print(key+": "+str(value))
