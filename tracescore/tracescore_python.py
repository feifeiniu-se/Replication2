...
# from BF import BF
from tracescore.BF_python import BF
...
def calculate(issues, filePath):
    ...
    for i in range(len(test_bugs)):
        ...
        issue.artifacts = [x for x in within365 if (len(set(f.new_filePath for f in x.files if f.new_filePath!="/dev/null" and f.new_filePath is not None)) <= 10) or (len(set(f.new_filePath for f in x.files if f.new_filePath!="/dev/null" and f.new_filePath is not None)) <= 20)]
        # 余弦相似度
